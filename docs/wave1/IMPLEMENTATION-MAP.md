...
| 6 | `TheBridge/Server/ToolRouter.swift` | Dispatch context struct gains `{transportSessionId?, origin: .local/.remote}`; `dispatchFormatted()` consults SessionRegistry → advisory `governance` annotation + `governed:false` audit flag; control-plane blocklist check before SecurityGate when `origin == .remote`. |
