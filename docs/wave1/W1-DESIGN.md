...
- **Broker-first tools/list ordering:** every MCP `tools/list` surface prioritizes `bridge_initialize`, `bridge_status`, `tools_list`, and `session_info` before the rest of the catalog while preserving non-bootstrap relative order. This keeps the broker reachable for clients with large-manifest import caps or stale-import refresh behavior.
