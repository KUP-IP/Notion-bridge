...
- 2026-09-04 · Runtime Exposure optional Deprecation Date + orphan purge · `3942 → 3947` (+5): missing SKILLS `Deprecation Date` warns instead of `schema_missing` hard-fail; `skills_exposure_purge_orphans` drops named local+published orphans after a lifecycle decision; `outreach-dispatch` HOLD is refused by generic sweep. +4 SkillExposureAuthorityTests +1 SkillsModuleTests. CI measured 3947 passed, 0 failed on `cursor/runtime-exposure-reconcile-c275` after rebase onto #252 (run 33903590254).
