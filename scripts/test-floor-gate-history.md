...
- 2026-09-06 · calendar free/busy v0 (Isaiah GO) · `3989 → 4012` (+23): read-only `calendar_free_busy` on EventKit FOCUS default; half-open `[start, end)` overlap (touching edges exclusive); fail-closed missing calendar / permission / bad date (never empty-busy-as-free). +22 CalendarModuleTests + 1 access-denied nested. CI must re-measure.
