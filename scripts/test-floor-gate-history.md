...
- 2026-09-08 · #254 skill files catalog + materialize · `4038 → 4058` (+20): Skills seed binds `files` / `googleDriveFile` / `manager`; `fetch_skill` emits `files[]` (empty-honest) + optional `assetRoot`; `skill_materialize_file` copies Notion-hosted attachments into `skill-files/<uuid>/` for `file_read`. Notion is not binary SSOT. +17 FetchSkillNotionFilesTests +2 RegistryConfigTests +1 SkillsModuleTests. `staticFeatureModuleToolCount` 239 → 240. Linux host cannot compile macOS 26; CI must re-measure.
