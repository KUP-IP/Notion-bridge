...
// NotionGate · Tests/IntegrationTests
//
...
func runEndToEndTests() async {
    ...
    await test("E2E: Cross-module — shell_exec output is valid structured response") {
        let result = try await router.dispatch(
            toolName: "shell_exec",
            arguments: .object(["command": .string("echo 'hello notiongate'")])
        )
        if case .object(let dict) = result {
            if case .string(let stdout) = dict["stdout"] {
                try expect(stdout.contains("hello notiongate"), "stdout should contain command output")
            } else {
    ...
    await test("E2E: Cross-module — file_write then file_read round-trip") {
        let testDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("notiongate-e2e-\(UUID().uuidString)")
        let testFile = testDir.appendingPathComponent("roundtrip.txt")
        let testContent = "NotionGate integration test: \(Date().ISO8601Format())"
